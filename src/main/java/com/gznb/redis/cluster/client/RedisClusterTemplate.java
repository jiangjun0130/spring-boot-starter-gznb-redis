package com.gznb.redis.cluster.client;
...
import com.gznb.redis.cluster.exception.RedisClientException;
 ...
 * @create 2017/9/15
 */
public class RedisClusterTemplate implements RedisOperations {

    private JedisCluster jedisCluster;

    public RedisClusterTemplate(JedisCluster jedisCluster) {
    ...
    public <T> Long setData(String key, T bean, Class<T> clazz, Integer seconds) throws RedisClientException {
        ...
        try {
            ...
            byte[] bytes = ProtobufIOUtil.toByteArray(bean, schema, LinkedBuffer.allocate(LinkedBuffer.DEFAULT_BUFFER_SIZE));
            // 删除原内容
    ...
    @Override
    public <T> Long setHashData(String key, Map<String, T> map, Class<T> clazz, Integer seconds) throws RedisClientException {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                T v = entry.getValue();
                byte[] bytes = ProtobufIOUtil.toByteArray(v, schema, LinkedBuffer.allocate(LinkedBuffer.DEFAULT_BUFFER_SIZE));
    ...
    @Override
    public <T> Long setListData(String key, List<T> dataList, Class<T> clazz, Integer seconds) throws RedisClientException {
        ...
        try {
            for (int i = 0; i < dataList.size(); i++) {
                byte[] beanBytes = ProtostuffIOUtil.toByteArray(dataList.get(i), schema, LinkedBuffer.allocate(LinkedBuffer.DEFAULT_BUFFER_SIZE));
                bytes[i] = beanBytes;
            }
    ...
    public <T> Long rpush2List(String key, Class<T> clazz, T... beans) throws RedisClientException {
        ...
        try {
            ...
            for (int i = 0; i < beans.length; i++) {
                byte[] bytes = ProtobufIOUtil.toByteArray(beans[i], schema, LinkedBuffer.allocate(LinkedBuffer.DEFAULT_BUFFER_SIZE));
                listBytes[i] = bytes;
            }
    ...
    public Long setCollectionData(String key, HashSet<String> set) throws RedisClientException {
        ...
        try {
            ...
            throw new RedisClientException("redisCluster client setCollectionData error！error info:" + e.getMessage(), e);
        }
    }
    ...
    public Set<String> getCollectionData(String key) throws RedisClientException {
        ...
        try {
            ...
            throw new RedisClientException("redisCluster client getCollectionData error！error info:" + e.getMessage(), e);
        }
    }
    ...
    public Long removeFromCollection(String key, Set<String> set) throws RedisClientException {
        ...
        try {
            ...
            throw new RedisClientException("redisCluster client removeFromCollection error！error info:" + e.getMessage(), e);
        }
    }
